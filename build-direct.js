// build-direct.js - Direct build script that bypasses all problematic features
...
console.log('\n🚀 Starting direct build with all bypasses...');
if (runCommand('npx next build --no-lint --no-mangling', 'Direct build failed')) {
  console.log('🎉 Direct build completed successfully!');
} else {
  console.error('❌ Direct build failed');
  console.error('\n🔍 The issue is Windows permission errors. You need to:');
  console.error('1. Run PowerShell as Administrator');
  console.error('2. Then run: npm run build:direct');
  process.exit(1);
}
